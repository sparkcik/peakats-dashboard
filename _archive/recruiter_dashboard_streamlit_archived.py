...
def update_reject_reason(candidate_id, reason):
    """Update reject reason for a rejected candidate"""
    conn = get_connection()
    if not conn:
        return
    conn.execute(text("""
...
# Status taxonomy
STATUS_OPTIONS = ['Active', 'On Deck', 'Followup', 'Intake', 'Unranked', 'Hired', 'Rejected']

REJECT_OPTIONS = {
    'Select reason...': None,
    'Ineligible': 'fadv_ineligible',
    'Duplicate': 'duplicate',
    'MIA / Withdrew': 'candidate_withdrew',
    'Low RWP': 'low_rwp',
}
...
def main():
    ...
    with st.sidebar:
        ...
        status_filter = st.selectbox(
            "Recruiting Status",
            statuses,
            help="Track recruiting pipeline status"
        )
    ...
    with tab1:
        ...
        if len(filtered_df) == 0:
            ...
            # Store original IDs and raw values
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    '_m_raw': None,
                    '_flag_raw': None,
                    '🚩': st.column_config.TextColumn('🚩', help='FADV status changed', width='small'),
                    'FedEx ID': st.column_config.TextColumn('FedEx ID', help='Enter FedEx ID when assigned', max_chars=20, width='small'),
                    'Notes': st.column_config.TextColumn('Notes', help='Click to edit recruiter notes (auto-clears flag)', max_chars=500, width='large'),
                    'G': st.column_config.CheckboxColumn('G', help='GCIC Form uploaded to FADV', default=False),
                    'M': st.column_config.CheckboxColumn('M', help='MEC Form uploaded to FADV', default=False)
                }
            )
            ...
            if edited_df is not None:
                ...
                for idx in range(len(edited_df)):
                    ...
                    if original_notes_full == 'nan': original_notes_full = ''
                    if edited_notes == 'nan': edited_notes = ''
                    ...
                    if original_fedex == 'nan': original_fedex = ''
                    if edited_fedex == 'nan': edited_fedex = ''
            ...
            with col_refresh:
                if st.button("🔄 Refresh Data", use_container_width=True):
                    st.rerun()
            with col_tip:
                st.caption("💡 Tip: ⚠️ = FADV status changed. Edits save instantly. Click 'Refresh Data' to see latest.")
            
            # Candidate detail selector
            st.divider()
    ...
    with tab2:
        ...
        with col3:
            ...
            form_data = pd.DataFrame({
                'Form': ['GCIC (G)', 'MEC (M)'],
                'Count': [len(filtered_df[filtered_df['g'] == 1]), len(filtered_df[filtered_df['m'] == 1])]
            })
    ...
    with tab3:
        ...
        st.download_button(
            label="📥 Download CSV",
            data=csv,
            file_name=f"PEAKATS_Export_{timestamp}.csv",
            mime="text/csv",
            use_container_width=True
        )
    ...
    if st.session_state.show_detail and st.session_state.selected_candidate:
        ...
        with st.sidebar:
            ...
            with st.expander("🏷️ Update Status", expanded=True):
                current_status = candidate['status'] if candidate['status'] in STATUS_OPTIONS else 'Active'
                ...
                # Reject reason — only visible when Rejected
                reject_reason = None
                if new_status == 'Rejected':
                    selected_label = st.selectbox(
                        "Rejection Reason",
                        list(REJECT_OPTIONS.keys()),
                        key=f"reject_reason_{candidate['id']}"
                    )
                    reject_reason = REJECT_OPTIONS[selected_label]
                
                if st.button("💾 Update Status", use_container_width=True):
                    update_candidate_status(candidate['id'], new_status)
                    if new_status == 'Rejected' and reject_reason:
                        update_reject_reason(candidate['id'], reject_reason)
